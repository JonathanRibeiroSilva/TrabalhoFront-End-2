...
{
    let comparador=document.getElementById('comparar');

    let intervalo=document.getElementById('intervalo');

    let atual=document.getElementById('atual');

    comparador.addEventListener('click', function(){
        ...
        if(!validar(data1,data2)){
            ...
            alert(`A data maior é: ${conversorData(comparaData(data1,data2))}`);
        }
    });
    ...
    atual.addEventListener('click', function(){
        ...
        alert(`A data atual é: ${conversorData(dataAtual)}`); 
    });
});
...
function conversorData(dataAtual){
